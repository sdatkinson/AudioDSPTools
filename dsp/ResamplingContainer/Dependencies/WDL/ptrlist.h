...
{
public:
  explicit WDL_PtrList(int defgran = 4096)
  : m_hb(defgran WDL_HEAPBUF_TRACEPARM("WDL_PtrList"))
  {
  }
  ...
  PTRTYPE* Get(INT_PTR index) const
  {
    ...
    if (list && (UINT_PTR)index < (UINT_PTR)(m_hb.GetSize() / sizeof(PTRTYPE*)))
      return list[index];
    return NULL;
  }
  ...
  void Delete(int index)
  {
    ...
    {
      if (index < --size)
        memmove(list + index, list + index + 1, (unsigned int)sizeof(PTRTYPE*) * (size - index));
  ...
  int DeleteBatch(bool (*proc)(PTRTYPE* p, void* ctx),
                  void* ctx = NULL) // proc returns true to remove item. returns number removed
  {
    ...
    if (cnt < sz)
      m_hb.Resize(cnt * sizeof(PTRTYPE*), false);
    return sz - cnt;
  }
...
template <class PTRTYPE>
class WDL_PtrList_DeleteOnDestroy : public WDL_PtrList<PTRTYPE>
{
public:
  explicit WDL_PtrList_DeleteOnDestroy(void (*delfunc)(void*) = NULL, int defgran = 4096)
  : WDL_PtrList<PTRTYPE>(defgran)
  , m_delfunc(delfunc)
  {
  }
  ~WDL_PtrList_DeleteOnDestroy() { WDL_PtrList<PTRTYPE>::EmptySafe(true, m_delfunc); }
